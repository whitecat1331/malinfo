...
class OutputToScreen(Output):
    ...
    def _display_output_header(self) -> None:
        ...
        self.info.update({self._frame.packet_num: local_time})
    ...
    def _display_ethernet_data(self) -> None:
        ...
        self.info.update({
            "Ethernet": f"{ethernet.src:.>23} -> {ethernet.dst}",
            "Interface": interface,
    ...
    def _display_ipv4_data(self) -> None:
        ...
        self.info.update({
            "IPv4": f"{ipv4.src:.>27} -> {ipv4.dst: <15}",
            "DSCP": ipv4.dscp,
            "Total_Length": ipv4.len,
            "ID": ipv4.id,
            "Flags": {ipv4.flags_str},
            "TTL": ipv4.ttl,
            "Protocol": ipv4.encapsulated_proto,
            "Header_Checksum": ipv4.chksum_hex_str
        })
    ...
    def _display_ipv6_data(self) -> None:
        ...
        self.info.update({
            "IPv6": f"{ipv6.src:.>27} -> {ipv6.dst: <15}",
            "Traffic_Class": ipv6.tclass_hex_str,
            "Flow_Label": ipv6.flabel_txt_str,
            "Payload_Length": ipv6.payload_len,
            "Next_Header": ipv6.encapsulated_proto,
    ...
    def _display_arp_data(self) -> None:
        ...
        if arp.oper == 1:  # ARP Request
            ...
            self.info.update(
                {"ARP_Request": f"ARP Who has {arp.tpa:.>18} ? -> Tell {arp.spa}"})
        else:              # ARP Reply
            print(f"{i}[+] ARP {arp.spa:.>28} -> Is at {arp.sha}")
            self.info.update(
                {"ARP_Reply": f"ARP {arp.spa:.>28} -> Is at {arp.sha}"})
        ...
        self.info.update({
            "Hardware_Type": arp.htype,
            "Protocol_Type:": f"{arp.ptype_str} {arp.ptype_hex_str}",
            "Hardware_Length": arp.hlen,
            "Protocol_Length": arp.plen,
            "Operation": f"{arp.oper} ({arp.oper_str})",
    ...
    def _display_udp_data(self) -> None:
        ...
        self.info.update({
            "UDP": f"{udp.sport:.>28} -> {udp.dport}",
            "Header_Length": udp.len,
            "Header_checksum": udp.chksum
        })
    ...
    def _display_icmpv4_data(self) -> None:
        ...
        self.info.update({
            "ICMPv4": f"{ipv4.src:.>27} -> {ipv4.dst: <15}",
            "ICMP_Type": f"{icmpv4.type} ({icmpv4.type_str})",
            "Header_Checksum": icmpv4.chksum_hex_str
        })
    ...
    def _display_icmpv6_data(self) -> None:
        ...
        self.info.update({
            "ICMPv6": f"{ipv6.src:.>27} -> {ipv6.dst: <15}",
            "Control_Message_Type": f"{icmpv6.type} {icmpv6.type_str}",
            "Control_Message_Subtype": icmpv6.code,
            "Header_Checksum": icmpv6.chksum_hex_str
        })
