...
*Max data (available) per ICMP packet 65507 bytes:
    ...
    def pack_request(self, data=None):
        ...
        """
        self.type = self.REQUEST_TYPE.to_bytes(
            Sizes.ICMP_TYPE, byteorder=byteorder)
        self.code = self.REQUEST_CODE.to_bytes(
            Sizes.ICMP_CODE, byteorder=byteorder)
        self.checksum = self.REQUEST_CHECKSUM.to_bytes(
            Sizes.ICMP_CHECKSUM, byteorder='big')
        new_identifier = int((id(data) * random()) % 65535)
        self.id = new_identifier.to_bytes(
            Sizes.ICMP_IDENTIFIER, byteorder=byteorder)
        self.seq = self.REQUEST_SEQUENCE.to_bytes(
            Sizes.ICMP_SEQUENCE, byteorder='big')
        if not data:
            curTime = int(timenow())
            timestamp = curTime.to_bytes(
                Sizes.ICMP_TIMESTAMP, byteorder=byteorder)
        ...
        self.checksum = newchecksum.to_bytes(
            Sizes.ICMP_CHECKSUM, byteorder='big')
    ...
    def pack_response(self, request, data=None):
        ...
        self.type = self.RESPONSE_TYPE.to_bytes(
            Sizes.ICMP_TYPE, byteorder=byteorder)
        self.code = self.RESPONSE_CODE.to_bytes(
            Sizes.ICMP_CODE, byteorder=byteorder)
        self.checksum = self.RESPONSE_CHECKSUM.to_bytes(
            Sizes.ICMP_CHECKSUM, byteorder='big')
        self.id = request.id
