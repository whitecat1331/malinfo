...
def main(shareName=None, sharePath=None,
         comment=None, username=None,
         password=None, hashes=None,
         ts=None, ip='0.0.0.0',
         port=445, smb2support=False):

    options = get_arguments()

    options.shareName = shareName
    options.sharePath = sharePath
    ...
    options.ip = ip
    options.port = port
    options.smb2support = smb2support

    start_smb_server(options)
...
def get_arguments():
    ...
    parser.add_argument('-shareName', action='store',
                        help='name of the share to add')
    parser.add_argument('-sharePath', action='store',
                        help='path of the share to add')
    parser.add_argument('-comment', action='store',
                        help='share\'s comment to display when asked for shares')
    parser.add_argument('-username', action="store",
                        help='Username to authenticate clients')
    ...
    parser.add_argument('-ip', '--interface-address', action='store',
                        default='0.0.0.0', help='ip address of listening interface')
    parser.add_argument('-port', action='store', default='445',
                        help='TCP port for listening incoming connections (default 445)')
    parser.add_argument('-smb2support', action='store_true',
                        default=False, help='SMB2 Support (experimental!)')
...
def start_smb_server(options):

    if not options.shareName and options.sharePath:
        parser.print_help()
        sys.exit(1)
    ...
    server = smbserver.SimpleSMBServer(
        listenAddress=options.interface_address, listenPort=int(options.port))
    ...
    # Rock and roll
    server.start()   # Init the example's logger theme
